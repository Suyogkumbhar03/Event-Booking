...
const seedDB = async () => {
  try {
    const mongoUri = process.env.MONGO_URI || 'mongodb://localhost:27017/eventora';
    ...
    if (!adminUser) {
      adminUser = await User.create({
        name: 'Eventora Editorial Admin',
        email: 'admin@eventora.org',
    ...
    // Clear existing events and seed fresh ones
    await Event.deleteMany({});
    console.log('[SEED] Cleared existing events');

    const eventsToCreate = sampleEventsData.map(evt => ({
      ...evt,
      organizer: adminUser._id
    }));

    const createdEvents = await Event.create(eventsToCreate);
    console.log(`[SEED] Successfully seeded ${createdEvents.length} editorial events into MongoDB.`);
    process.exit(0);
  } catch (error) {
